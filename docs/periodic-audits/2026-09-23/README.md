...
| **12** | تدفقات Telegram والتوافق الوظيفي | 6 | 68 | 4.08 | ⏸️ **مؤجلة بقرار استراتيجي (إعادة هيكلة شاملة)** | [`12-telegram-flows-and-parity.md`](./12-telegram-flows-and-parity.md) |
...
### 🚨 ملخص الإخفاقات والقرارات الاستراتيجية للمالك (Owner Decisions):
1. **[STRATEGIC DECISION] تأجيل معالجة إخفاقات العرض الموحد وأطوال الأزرار:** تم تأجيل إصلاح الـ 41 استدعاءً لرسائل النصوص الخام وأزرار الجوال بقرار معتمد من المالك، لدمجها ضمن خطة عمل كبرى لإعادة هيكلة وتوحيد كافة تدفقات البوت الـ 126 دفعة واحدة وفق محرر النصوص الغنية (`rich-message`) ومنع الترقيع المؤقت.
2. **[MAJOR] غياب أتمتة النسخ الاحتياطي والاستعادة:** انعدام وجود أي سكريبت أو حاوية مجدولة لـ `pg_dump` مع غياب اختبارات الاستعادة الدورية (مدرجة كأولوية في خطة العمل 97 القادمة).
3. **[MAJOR] 12 ثغرة أمنية في التبعيات (`pnpm audit`):** وجود 5 ثغرات بمستوى خطورة High و7 بمستوى Moderate (جاهزة للمعالجة عبر pnpm.overrides).
4. **[MAJOR] خطأ SQL في كود تنظيف بيئة الاختبار المالي:** استعلام خاطئ عن عمود `voucherNumber` بدلاً من `voucherId` في جدول `hospitality_expenses` (جاهز للتصحيح الفوري).
