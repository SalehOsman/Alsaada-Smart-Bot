...
> **Goal:** بناء منهجية النقل المعيارية، وتثبيت النواة تشفيرياً، وتمكين إضافة الموديولات والوظائف من ملفاتها الخاصة مع الاكتشاف التلقائي والذكاء النمطي (TypeSafe AI) أثناء البناء والنشر، دون تعديل مصادر النواة بعد اعتمادها، وتجهيز المشروع لاستقبال تدفقات F:\HR الـ 126 تباعاً لاحقاً.
> **Architecture:** معمارية مزدوجة متكاملة (Dual-Engine TypeSafe): نواة ثابتة بعقود إصدارية وأمان نمطي صارم (TypeScript/Zod/Prisma DMMF)، مدعومة بمحرك ذكاء لحظي فائق السرعة (TypeSafe AI System One / Jev-1.13) يتولى توجيه التدفقات، استدعاء الدوال، حراسة السياسات، ومطابقة الكيانات.
> **Tech Stack:** تقنيات المشروع: TypeScript 5.9+، pnpm workspace، TypeSafe AI SDK (Jev-1.13)، grammY، Next.js/React، Prisma/PostgreSQL، Redis، Vitest، Docker.
...
9. **حصر النطاق في بناء المنهجية والنواة:** قرر المستخدم صراحة ألا يتم نقل تدفقات `F:\HR` في هذه الخطة، بل يقتصر العمل على **إنشاء منهجية النقل وتجهيز النواة كمنصة استقبال ثابتة ومحكمة 100%**، على أن يبدأ نقل التدفقات الـ 126 تباعاً في خطط مستقلة لاحقة.
10. **الاعتماد الأقصى على منصة TypeSafe AI (System One / Jev-1.13):** دمج ترسانة المنصة (`skill_suggestion`, `function_calling`, `autoresearch_feature_discovery`, `entity_alignment`, `llm_guardrails`) لضمان أقصى سرعة استجابة (<100ms) ودقة متناهية وصفر هشاشة.
...
- عقود الموديول والوظيفة وواجهات الخدمة والصلاحيات وتوافق الإصدارات (الإصدار 2).
- تكامل منصة TypeSafe AI (System One / Jev-1.13) وترسانة الـ Cookbooks (Skill Suggestion، Function Calling Dispatcher، Entity Alignment، LLM Guardrails، Autoresearch).
- الاكتشاف والتحقق والتجميع الحتمي ومخرجاته المنفصلة.
- ربط البوت والقوائم والجلسات، وربط صفحات الداشبورد وواجهات API.
- تركيب مخطط Prisma وإدارة ترحيلات الموديولات وتوليد العميل عبر فاحص DMMF AST.
- تحديث المولّدات لتكتب داخل نطاق الموديول فقط وتنتج شرائح الـ 10 ملفات الدستورية.
- إعداد بناء workspace وDocker وCI والتسريع التزايدي (<500ms) ليشمل أي موديول مطابق دون تسجيل مركزي يدوي.
- تكييف الموديولات الحالية دون تغيير السلوك أو إعادة إنشاء البيانات.
- حماية النواة والموديول والوظيفة، وإصدار دستور نقل التدفقات وقوالب الأدلة والبطاقات.
- إثبات عملي لمنهجية النواة بإضافة موديول تجريبي معزول (`sample-domain`) وتدفقين تجريبيين (`89.1` و `89.2`) بعد غلق بنيته المشتركة.

### 4.2 خارج النطاق

- نقل أي تدفق حقيقي من تدفقات `F:\HR` الـ 126 في هذه الخطة؛ حيث يبدأ النقل الفعلي تباعاً وبشكل مستقل في خطط عمل تالية بمجرد اعتماد النواة ومنهجية النقل.
...
### 8.1 البوت والذكاء النمطي اللحظي (TypeSafe AI Integration)
...
- **توجيه التدفقات عبر TypeSafe Skill Suggestion:** تطبيق نمط Progressive Disclosure لمسح وتوجيه الـ 126 تدفقاً مستقبلياً بمرحلتين خفيفتين (Call 1 Skim عبر Choice و Noul + Call 2 Verify على أفضل 3 مرشحين) لخفض أخطاء التوجيه بنسبة تتجاوز 50% وتوفير آلاف التوكنز في أجزاء من الثانية (<100ms).
- **معالجة الإجراءات عبر TypeSafe Function Calling:** تحويل مدخلات المستخدم باللغة الطبيعية إلى استدعاءات برمجية نمطية لدوال معالجات التدفق بمدخلات مغلقة (`Literal`) مع حساب مؤشر الثقة (Confidence Score).
- **دروع الحماية السيادية عبر TypeSafe LLM Guardrails:** فحص كل مدخل ومخرج ببطارية أسئلة `Noul` و `Score` لضمان عدم تسريب الرواتب (G8)، ومنع تجاوز الصلاحيات (G7)، وحماية الثوابت المالية والمحاسبية (G11, G12).
...
### 8.2 الداشبورد والبحث الدلالي (Semantic Find & Rerank)
...
- تزويد الداشبورد بالبحث الدلالي الفوري (`semantic_find`) وإعادة الترتيب التلقائي (`rerank_typesafe`) للقوائم والشاشات حسب دور المستخدم دون الحاجة لبنية تحتية ثقيلة لقواعد بيانات متجهة.
- حراسة حدود تسلسل البيانات عبر `JsonSerializable<T>` لضمان عدم تسريب كائنات الخادم أو دوال Prisma إلى Client Components في المتصفح.
...
- [ ] تشغيل حلقة **`autoresearch_feature_discovery`** (منصة TypeSafe AI) لفحص ومقارنة شاشات وأكواد `F:\HR` واكتشاف الفجوات والخدمات المشتركة المطلوبة لاحقاً للتدفقات الـ 126، وتوليد `core-capability-matrix.md` آلياً لسد أي نقص قبل إغلاق النواة.
...
**إنشاء:** `packages/core-components/src/contracts/module-v2.contract.ts`، `flow-v2.contract.ts`، `capability.contract.ts`، `typesafe-primitives.contract.ts`، `docs/schemas/module-v2.schema.json`، `docs/schemas/flow-v2.schema.json`، `packages/core-components/tests/module-v2-contract.spec.ts`.
...
- [ ] دمج مواصفات أسئلة TypeSafe (`Choice`, `Noul`, `Score`) كجزء صلب من `flow-v2.contract.ts` مع دعم الـ Nominal Branded Types (`ModuleId`, `FlowId`).
...
### P3 — تكامل البوت والجلسات والقوائم الذكية (TypeSafe Router)
...
**إنشاء:** `packages/core-components/src/module-bus/catalog-adapter.ts`، `packages/core-components/src/module-bus/typesafe-flow-router.ts`، `apps/bot-server/tests/catalog-bot-integration.spec.ts`، `apps/bot-server/tests/catalog-session-routing.spec.ts`.

- [ ] دمج `TypeSafe Skill Suggestion` لتوجيه التدفقات المستقبلية بأسلوب Progressive Disclosure وخفض معدل الخطأ بأكثر من 50% بسرعة <100ms.
- [ ] دمج `TypeSafe Function Calling Dispatcher` لتحويل أوامر البوت إلى دوال TypeScript صلبة ذات Confidence Score وإلزام المعالجات بـ `assertNever(step: never)`.
...
**الخروج:** البوت يكتشف موديولًا جديدًا ووظيفة لاحقة من catalog مع محرك ذكاء نمطي فوري وتوجيه آمن.
...
### P5 — تركيب البيانات وترحيلاتها (DMMF AST & Entity Alignment)
...
- [ ] دمج أداة **`TypeSafe Entity Alignment`** لمطابقة تعريفات الجداول والحقول القديمة من `F:\HR` بنماذج Prisma الحديثة وضمان التوافق النمطي.
- [ ] حفظ schema baseline وDDL لقواعد اختبار؛ اختبار نقل التعريفات دون اختلاف database shape أو checksum للترحيلات القائمة.
- [ ] تركيب fragments وفحص AST/structure عبر Prisma DMMF Validator وإضافات العلاقات المسموحة؛ رفض duplicate model/table والعبث بحقول نواة.
...
### P9 — إنفاذ الغلق وحراسة الأمان النمطي (Immutability Sentinel & LLM Guardrails)
...
**إنشاء:** `tools/governance/verify-core-immutability.ts`، `tools/governance/typesafe-guardrails.ts`، `verify-module-boundaries.ts`، `tests/core-immutability.spec.ts` و`tests/module-boundaries.spec.ts` تحت `tools/governance/`، `docs/ai-execution-evidence/wp-89/core-baseline.json`.

- [ ] تفعيل دروع **`TypeSafe LLM Guardrails`** كحارس سيادي دائم لفحص المعاملات المالية وتعديل الرواتب والصلاحيات (G7, G8, G11, G12) بأسئلة `Noul` و `Score`.
...
**الخروج:** تعديل مصدر نواة يفشل، وإضافة وظيفة مطابقة تمر دون فتح النواة مع حراسة ذكية للسياسات.

### P10 — دستور النقل وقوالب الإثبات (TypeSafe Migration Constitution)
...
- [ ] إصدار **دستور نقل التدفقات** المعتمد على منصة TypeSafe AI، وتوفير بطاقات الهجرة المعيارية (`flow-migration-card.md`) لتمكين النقل التتابعي اللاحق للـ 126 تدفقاً بنظام Plug-and-Play.
