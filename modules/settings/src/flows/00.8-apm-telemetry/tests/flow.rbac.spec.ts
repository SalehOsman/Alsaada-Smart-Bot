...
describe('Flow 00.8 RBAC Tests — رادار الأداء ومراقبة الخدمات', () => {
  beforeEach(() => {
  ...
  it('denies worker role attempting APM dashboard access', async () => {
    // Arrange
    ...
    // Act
    await handler.renderApmDashboard(ctxWorker);
  ...
  it('authorizes verified Super Admin access unconditionally', async () => {
    ...
    // Act
    await handler.renderApmDashboard(ctxSuper);

    // Assert
    expect(replyMock).toHaveBeenCalledWith(
      expect.stringContaining('رادار قياس الأداء والسرعة الفائقة'),
