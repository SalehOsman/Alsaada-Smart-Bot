...
### المزايا التشغيلية:
1. **الوصول الآمن والتشغيل المدار (Prisma 7 Studio Engine):**
   - يعمل محرك Studio وفق معايير **Prisma 7.10.0** مع تمرير ملف التكوين السيادي: `prisma studio --config packages/database/prisma.config.ts`.
   - إدارة الجلسات عبر `StudioProcessManager` المدمج في لوحة التحكم الإدارية (`apps/admin-dashboard`).
   - رقيب الخمول التلقائي (`15-Minute Watchdog Timer`): إيقاف تشغيل الخادم تلقائياً عند انقطاع النشاط لمدة 15 دقيقة لحماية موارد السيرفر والمقابس.
