...
### 1️⃣ الهيكل العشاري المعياري للتدفقات الوظيفية (The 10-File Vertical Slice Standard)
لكل تدفق وظيفي داخل مجلد `modules/<module>/src/flows/<flow-slug>/` بنية رأسية معزولة تتضمن الملفات العشرة المعيارية التالية:

```text
modules/<module>/src/flows/<flow-slug>/
├── flow.contract.json       # عقد التدفق، مخطط الحالات، والأدوار المصرح لها (RBAC Matrix)
├── index.ts                 # نقطة الدخول وتصدير التدفق للموديول والبوت
├── controller.ts            # منسق التدفق، استقبال الأحداث، وإدارة المراحل
├── menu.builder.ts          # بناء لوحات المفاتيح وفق ميزانية الأزرار (36/16/7/3)
├── action.handler.ts        # معالجة أزرار التفاعل (Callbacks) والتحولات
├── service.ts               # منطق الأعمال، المعاملات، والحسابات (خالٍ تماماً من BotContext)
├── types.ts                 # الأنواع الصارمة، كائنات الجلسة، والبيانات
├── validator.ts             # التحقق من صحة المدخلات ومخططات Zod
├── error.handler.ts         # إدارة الأخطاء، رسائل الاستعادة، والتسجيل التليميتري
└── tests/
    └── <flow-slug>.spec.ts  # اختبارات الوحدة والتكامل بنمط AAA وعينات النفي
```

* **الضابط الهندسي الصارم:** يُمنع خلط كود الواجهات (UI) مع كود الخدمات (Services)؛ فملف `service.ts` لا يعرف شيئاً عن `BotContext` أو تليجرام، وتتوافق الملفات القديمة المسماة (`flow.*.ts`) كنسخة متوافقة مع هذا المعيار العشاري الإلزامي.
