---
title: "💰 المنظومة المالية المحاسبية المغلقة وقائمة الدخل والأرباح"
...
## 🔄 5. المنظومة المعيارية لمسحوبات وسلف العاملين (The 3-Way Advance & In-Kind Clearing)

للقضاء التام على فخ ازدواج المصروفات الميدانية (Double-Counting) الناتج عن سحب العاملين بضائع أو سجائر من مشتريات الشركة الموردة، تنقسم مسحوبات وسلف العاملين حصراً إلى **3 وظائف وتدفقات منفصلة تماماً**:

```mermaid
graph TD
    WorkerWithdrawal["مسحوبات وسلف العاملين"]
    
    WorkerWithdrawal --> Flow1["1️⃣ مسحوبات السجائر (Cigarettes)"]
    WorkerWithdrawal --> Flow2["2️⃣ مسحوبات المشتريات العينية (In-Kind Purchases)"]
    WorkerWithdrawal --> Flow3["3️⃣ السلف النقدية الكاش (Cash Advances)"]
    
    Flow1 --> InKind1["عيني من مخزون الكانتين<br/>(خروج نقدية = 0 ج.م)"]
    Flow2 --> InKind2["عيني من مشتريات الموردين<br/>(خروج نقدية = 0 ج.م)"]
    Flow3 --> CashOut["نقدي فعلي مع تحديد المصدر الإلزامي<br/>(عهدة موقع أو الخزينة الرئيسية)"]
    
    InKind1 --> Clearing1["🔄 قيد مقاصة: تخفيض مشتريات الكانتين لنفس القيمة"]
    InKind2 --> Clearing2["🔄 قيد مقاصة: تخفيض مشتريات الموردين لنفس القيمة"]
    CashOut --> DeductCash["خصم المبلغ من رصيد العهدة/الخزينة المحددة"]
    
    Clearing1 --> DeductSalary["خصم إجمالي المسحوبات والسلف من راتب العامل في مسير الرواتب الشهري"]
    Clearing2 --> DeductSalary
    DeductCash --> DeductSalary
```

### 1️⃣ مسحوبات السجائر (`WITHDRAWAL_CIGARETTES`):
* تدفق فائق السرعة عبر الكتالوج الرسمي المعتمد بالأسعار والكميات المحددة دون كسور عشرية.
* الأثر: لا يطلب البوت أي عهدة أو خزينة نقدية (`0.00 ج.م كاش`)، ولا يُعاد تسجيل استحقاق للمورد، بل يتم قيد مقاصة يخفض تكلفة مشتريات الكانتين للشركة ويقيد المبلغ كمسحوبات على العامل تُخصم من راتبه الشهري.

### 2️⃣ مسحوبات المشتريات العينية (`WITHDRAWAL_PURCHASES`):
* تدفق لتسجيل الأغراض والمشتريات الشخصية للعامل على حساب الشركة (مثل: مهمات حماية، بقالة، أدوية).
* يقدم البوت أزرار مقترحات سريعة مبنية على الأصناف الأكثر تكراراً مع إمكانية كتابة بيان مخصص يدوياً.
* الأثر: لا يخرج أي كاش من العهد (`0.00 ج.م كاش`)، ويتم قيد مقاصة يخفض تكلفة مشتريات الشركة من المورد ويحملها كمسحوبات على العامل.

### 3️⃣ السلف النقدية (`ADVANCE_CASH`):
* تدفق تسليم أموال ورقية كاش للعامل.
* **الضابط الإلزامي:** يُلزم البوت المشرف باختيار مصدر الأموال عبر `UniversalSourceOfFundsPicker` (قائمة العهد المفتوحة بالموقع برصيدها اللحظي، أو الخزينة الرئيسية للشركة).
* يتم التحقق التلقائي من كفاية الرصيد لمنع الصرف على المكشوف، ويُخفض المبلغ من رصيد العهدة/الخزينة المختارة.
