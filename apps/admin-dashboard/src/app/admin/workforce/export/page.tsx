  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-2xs space-y-6">
        {downloadSuccess && (
          <div className="bg-orange-50 border border-orange-300 text-orange-800 p-4 rounded-xl flex items-center gap-3">
            <CheckCircle2 className="w-5 h-5 text-orange-600 shrink-0" />
            <div>
              <p className="text-sm font-bold">تم تجهيز وتنزيل الملف بنجاح!</p>
              <p className="text-xs text-orange-700">الملف متطابق مع أعمدة الشيت المعتمدة ومحدث لحظياً.</p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">نطاق الموقع الميداني</label>
            <select
              value={site}
              onChange={(e) => setSite(e.target.value)}
              className="w-full text-xs px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 min-h-[44px]"
            >
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">الحالة التشغيلية للعمال المستهدفين</label>
            <select
              value={status}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1.5">صيغة الملف المطلوبة</label>
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => setFormat('XLSX')}
                className={`p-3 rounded-lg border text-xs font-semibold flex items-center justify-center gap-2 transition-all min-h-[44px] ${
                  format === 'XLSX'
        ...
        <div className="pt-4 border-t border-slate-200 flex items-center justify-end gap-3">
          <button
            ...
            className="w-full py-2.5 rounded-lg bg-orange-600 text-white text-xs font-bold hover:bg-orange-700 transition-colors shadow-2xs flex items-center justify-center gap-2 disabled:opacity-50 min-h-[44px]"
          >
