      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border border-slate-300 bg-white text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors shadow-2xs min-h-[44px]"
          >
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs flex flex-col md:flex-row items-center gap-4">
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="text-xs bg-white border border-slate-300 rounded-lg px-3 py-2 min-h-[44px] text-slate-700 focus:outline-hidden focus:ring-1 focus:ring-orange-500 flex-1 sm:flex-initial cursor-pointer"
          >
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-slate-900 bg-orange-50 text-orange-800 px-2.5 py-1 rounded-md border border-orange-200 text-sm">
                      {worker.nickname}
                    </span>
                    <span className="font-mono text-xs font-bold text-slate-600">{worker.code}</span>
                  </div>

                  <div>
                    <p className="text-xs font-semibold text-slate-900">{worker.fullName}</p>
                    <div className="flex flex-col gap-1 text-[11px] text-slate-600 mt-1.5 bg-slate-50 p-2 rounded-lg border border-slate-100 font-mono">
                      <div className="flex items-center gap-1.5">
                        <span className="text-slate-500 font-sans font-semibold">🪪 الرقم القومي:</span>
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-slate-500 font-sans font-semibold">📱 الهاتف:</span>
                  ...
                  <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-100">
                    ...
                    <span className="font-medium text-slate-800">{worker.jobTitle}</span>
                  </div>

                  <div className="flex flex-col gap-1 pt-2 border-t border-slate-100 text-xs">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 font-medium">الحالة الوظيفية:</span>
                      <div>{getStatusBadge(worker.status)}</div>
                    </div>
                    {canViewFinances && worker.totalMonthlySalary !== undefined && (
                      <div className="bg-orange-50/70 p-2 rounded-lg border border-orange-200/60 mt-1 space-y-1">
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-slate-600">الراتب الأساسي:</span>
                          <span className="font-mono font-bold text-slate-800">{worker.basicSalary || 0} ج.م</span>
                        </div>
                        <div className="flex items-center justify-between text-[11px]">
                          <span className="text-slate-600">البدلات والإضافي:</span>
                          <span className="font-mono font-bold text-slate-800">{worker.fixedAllowances || 0} ج.م</span>
                        </div>
                        <div className="flex items-center justify-between text-xs pt-1 border-t border-orange-200/60">
                          <span className="font-bold text-orange-950">إجمالي الراتب الشهري:</span>
                          <span className="font-mono font-bold text-orange-700">{worker.totalMonthlySalary} ج.م / شهر</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                    {worker.phone && worker.phone !== 'غير مسجل' && worker.phone !== '***' && (
                      <a
                        ...
                        className="p-2 rounded-lg text-green-700 bg-green-50 hover:bg-green-100 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center gap-1 text-xs font-semibold"
                    ...
                    <Link
                      ...
                      className="p-2 rounded-lg text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center gap-1 text-xs font-medium"
                    >
                    ...
                    <Link
                      ...
                      className="p-2 rounded-lg text-rose-600 bg-rose-50 hover:bg-rose-100 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center gap-1 text-xs font-medium"
                    >
                  ...
                  {filteredWorkers.map((worker) => {
                    ...
                    return (
                      <tr key={worker.id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-3.5 px-4 font-mono font-bold text-slate-900">{worker.code}</td>
                        <td className="py-3.5 px-4">
                          <span className="font-bold text-slate-900 bg-orange-50 text-orange-800 px-2 py-0.5 rounded-md border border-orange-200/60">
                            {worker.nickname}
                          </span>
                        </td>
                        <td className="py-3.5 px-4">
                          ...
                          <div className="flex items-center gap-1.5 text-[11px] font-mono mt-1">
                            <span className="text-slate-500 font-sans text-[10px]">🪪 الرقم القومي:</span>
                        ...
                        <td className="py-3.5 px-4">
                          <p className="font-medium text-slate-900">{worker.jobTitle}</p>
                          {canViewFinances && worker.totalMonthlySalary !== undefined && (
                            <div className="mt-1 text-[11px] space-y-0.5">
                              <div className="font-bold text-orange-700 font-mono">
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            {worker.phone && worker.phone !== 'غير مسجل' && worker.phone !== '***' && (
                              <a
                                ...
                                rel="noopener noreferrer"
                                title="إرسال إشعار عبر واتساب"
                                className="p-2 rounded-md text-green-700 hover:bg-green-50 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center"
                              >
                                <MessageSquareShare className="w-4 h-4" />
                              </a>
                            )}
                            <Link
                              ...
                              className="p-2 rounded-md text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center"
                            >
                              <Eye className="w-4 h-4" />
                            </Link>
                            <Link
                              ...
                              className="p-2 rounded-md text-orange-600 hover:bg-orange-50 hover:text-orange-800 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center"
                            >
                            ...
                            <Link
                              ...
                              className="p-2 rounded-md text-rose-500 hover:bg-rose-50 hover:text-rose-700 transition-colors min-h-[44px] min-w-[44px] inline-flex items-center justify-center"
                            >
                              <DoorOpen className="w-4 h-4" />
                            </Link>
                          </div>
