  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Tabs */}
        <div className="flex flex-wrap bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-semibold gap-1">
          <button
          ...
          {clearances.length === 0 ? (
            ...
            <>
              ...
              <div className="block md:hidden space-y-3">
                {clearances.map((c) => (
                  <div key={c.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs space-y-4">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-bold text-slate-900">{c.workerName}</h3>
                          <p className="text-xs text-slate-500">كود العامل: {c.workerCode}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                      <div className="bg-slate-50 p-3 rounded-lg border border-slate-200/60">
                        <span className="text-slate-400 block mb-1">مدة الخدمة</span>
                        <strong className="text-slate-900 text-sm font-mono">{c.workedDays} يوم</strong>
                      </div>
                      <div className="bg-slate-50 p-3 rounded-lg border border-slate-200/60">
                        <span className="text-slate-400 block mb-1">صافي المخالصة</span>
                        <strong className="text-orange-700 text-sm font-mono">
                          {c.netSettlementAmount.toLocaleString()} ج.م
                        </strong>
                      </div>
                      <div className="bg-slate-50 p-3 rounded-lg border border-slate-200/60">
                        <span className="text-slate-400 block mb-1">الحالة الحالية</span>
                        <strong className="text-slate-700 text-sm">{c.status}</strong>
                      </div>
                    </div>
                  </div>
                ))}
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {clearances.map((c) => (
                        <tr key={c.id} className="hover:bg-slate-50/80 transition-colors">
                          <td className="py-3.5 px-4 font-mono font-bold text-slate-900">{c.voucherId}</td>
                          <td className="py-3.5 px-4">
                            <p className="font-bold text-slate-900">{c.workerName}</p>
                            <p className="text-[11px] text-slate-400 font-mono mt-0.5">{c.workerCode}</p>
                          </td>
                          <td className="py-3.5 px-4 text-slate-600">{c.siteName}</td>
                          <td className="py-3.5 px-4 font-mono font-semibold text-slate-800">{c.workedDays} يوم</td>
                          <td className="py-3.5 px-4 text-slate-500 font-mono text-[11px]">{c.date}</td>
                          <td className="py-3.5 px-4 font-mono font-bold text-orange-700">
                            {c.netSettlementAmount.toLocaleString()} ج.م
                          </td>
                          <td className="py-3.5 px-4">
                            <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-orange-700 bg-orange-50 px-2.5 py-1 rounded-md border border-orange-200/60">
                              {c.status}
                            </span>
                          </td>
                        </tr>
                  ...
                  <div key={d.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h4 className="text-sm font-bold text-slate-900">{d.workerName}</h4>
                          <p className="text-xs text-slate-500">كود: {d.workerCode}</p>
                        </div>
                      </div>
                    ...
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-xs">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-semibold text-slate-700">القيمة المقررة: {d.amountOrDays}</span>
                      </div>
                      <p className="text-slate-600">السبب: {d.reason}</p>
                    </div>

                    <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                      <button
                        onClick={() => handleRejectDecision(d.id)}
                        className="min-h-[44px] min-w-[44px] px-3.5 py-2 rounded-lg border border-slate-300 text-xs font-semibold text-slate-600 hover:bg-slate-100 transition-colors inline-flex items-center justify-center cursor-pointer"
                      >
                        استبعاد / رفض
                      </button>
                      <button
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {decisions.map((d) => (
                        <tr key={d.id} className="hover:bg-slate-50/80 transition-colors">
                          ...
                          <td className="py-3.5 px-4">
                            <p className="font-bold text-slate-900">{d.workerName}</p>
                            <p className="text-[11px] text-slate-400 font-mono mt-0.5">{d.workerCode}</p>
                          </td>
                          <td className="py-3.5 px-4 font-mono font-bold text-slate-900">{d.amountOrDays}</td>
                          <td className="py-3.5 px-4 text-slate-600 max-w-xs truncate">{d.reason}</td>
                          <td className="py-3.5 px-4 text-slate-400 font-mono text-[11px]">{d.date}</td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-2xs space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div>
                <label className="font-semibold text-slate-700 block mb-1">العامل المستهدف:</label>
                <input
                  type="text"
                  value={calcWorker}
                  onChange={(e) => setCalcWorker(e.target.value)}
                  className="w-full border border-slate-300 rounded-lg p-2.5 min-h-[44px] font-medium"
                />
              </div>

              <div>
                <label className="font-semibold text-slate-700 block mb-1">أيام العمل الميدانية:</label>
                <input
                  type="number"
                  value={calcWorkedDays}
                  onChange={(e) => setCalcWorkedDays(Number(e.target.value))}
                  className="w-full border border-slate-300 rounded-lg p-2.5 min-h-[44px] font-mono font-bold"
                />
              </div>

              <div>
                <label className="font-semibold text-slate-700 block mb-1">أجر اليومية المعتمد (ج.م):</label>
                <input
                  type="number"
                  value={calcDailyWage}
                  onChange={(e) => setCalcDailyWage(Number(e.target.value))}
                  className="w-full border border-slate-300 rounded-lg p-2.5 min-h-[44px] font-mono font-bold text-orange-700"
                />
              </div>

              <div>
                <label className="font-semibold text-slate-700 block mb-1">إجمالي السلف والأقساط المستحقة (ج.م):</label>
                <input
                  type="number"
                  value={calcAdvances}
                  onChange={(e) => setCalcAdvances(Number(e.target.value))}
                  className="w-full border border-slate-300 rounded-lg p-2.5 min-h-[44px] font-mono font-bold text-rose-600"
                />
              </div>

              <div>
                <label className="font-semibold text-slate-700 block mb-1">الجزاءات المعتمدة (ج.م):</label>
                <input
                  type="number"
                  value={calcPenalties}
                  onChange={(e) => setCalcPenalties(Number(e.target.value))}
                  className="w-full border border-slate-300 rounded-lg p-2.5 min-h-[44px] font-mono"
                />
              </div>

              <div>
                <label className="font-semibold text-slate-700 block mb-1">استقطاعات تلفيات العهد / PPE (ج.م):</label>
                <input
                  type="number"
            ...
            <div className="pt-3 border-t border-slate-100 text-xs">
              <label className="font-semibold text-slate-700 block mb-2">خطة الصرف المالي:</label>
