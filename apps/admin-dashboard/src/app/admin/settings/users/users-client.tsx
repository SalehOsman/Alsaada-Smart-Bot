...
export function UsersClient({
  ...
  return (
    <div className="space-y-6">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                  {filteredUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="py-3.5 px-4 font-medium text-slate-900 dark:text-slate-100">{u.name || 'بدون اسم'}</td>
                      ...
                      <td className="py-3.5 px-4">
                        {u.status === 'ACTIVE' ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-slate-400 dark:text-slate-500 font-semibold">
                            <AlertCircle className="w-3.5 h-3.5" />
                            <span>معطل / مراجع</span>
                          </span>
                        )}
                      </td>
                      <td className="py-3.5 px-4 text-slate-400 dark:text-slate-500 font-mono text-[11px]">{u.lastActive}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                    ...
                    <tr key={d.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        {d.status === 'ACTIVE' ? (
                          ...
                          <span className="inline-flex items-center gap-1 text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded-full font-bold text-[11px]">
                            <XCircle className="w-3 h-3 text-slate-500" />
                            <span>ملغي (REVOKED)</span>
                          </span>
                        )}
                      </td>
                      <td className="py-3.5 px-4 font-mono text-[11px] text-slate-500 dark:text-slate-400">{d.startsAt}</td>
                      <td className="py-3.5 px-4 text-slate-600 dark:text-slate-400 text-xs max-w-xs truncate">{d.reason || '-'}</td>
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          {d.status === 'PENDING' && canManageDelegations && (
                            <>
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-orange-600 text-white rounded text-[11px] font-bold hover:bg-orange-700 transition-colors min-h-[36px] cursor-pointer"
                              >
                                اعتماد
                              </button>
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded text-[11px] font-semibold hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors min-h-[36px] cursor-pointer"
                              >
                                رفض
                              </button>
                            </>
                          ...
                          {d.status === 'ACTIVE' && (
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800/50 rounded text-[11px] font-bold hover:bg-rose-100 dark:hover:bg-rose-900/40 transition-colors min-h-[36px] cursor-pointer"
                            >
                              سحب التفويض
                            </button>
                          )}
      ...
      {activeTab === 'legacy_review' && (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-2xs space-y-4 p-4">
          <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 text-amber-900 dark:text-amber-300 text-xs leading-relaxed">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                {legacyAwaitingUsers.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-mono text-slate-600 dark:text-slate-400">{u.telegramId}</td>
                    <td className="py-3.5 px-4">
                      <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200/50 dark:border-slate-700 px-2 py-0.5 rounded font-mono text-[11px]">
                        {u.role} (معلق)
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400">{u.assignedSite}</td>
                  </tr>
                ))}
              </tbody>
            </table>
