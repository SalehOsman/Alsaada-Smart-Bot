      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">إجمالي المعلق</span>
            <Clock className="w-4 h-4 text-orange-500 dark:text-orange-400" />
          </div>
          <p className="text-2xl font-black text-slate-900 dark:text-slate-100 mt-2 font-mono">{counts.all}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">طلبات بانتظار القرار السيادي</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">مخالصات الخدمة</span>
            <FileCheck className="w-4 h-4 text-rose-500 dark:text-rose-400" />
          </div>
          <p className="text-2xl font-black text-rose-700 dark:text-rose-400 mt-2 font-mono">{counts.clearance}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">تصفية مستحقات وإنهاء</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">السلف النقدية</span>
            <DollarSign className="w-4 h-4 text-amber-500 dark:text-amber-400" />
          </div>
          <p className="text-2xl font-black text-amber-700 dark:text-amber-400 mt-2 font-mono">{counts.advance}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">تجاوزات وسلف ميدانية</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">الإجازات الميدانية</span>
            <Calendar className="w-4 h-4 text-blue-500 dark:text-blue-400" />
          </div>
          <p className="text-2xl font-black text-blue-700 dark:text-blue-400 mt-2 font-mono">{counts.leave}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">نزول وعودة مواقع</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:hidden">
            {filtered.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs space-y-4"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-mono font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded border border-transparent dark:border-slate-700">
                      {item.ticketNumber}
                    </span>
                    <span className="text-[11px] font-bold text-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/50 px-2 py-0.5 rounded border border-orange-200 dark:border-orange-900/60">
                      {item.typeLabel}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    {item.amount !== undefined && item.amount > 0 && (
                      <span className="font-bold text-orange-700 dark:text-orange-400 font-mono text-sm">
                        {item.amount} ج.م
                      </span>
                    )}
                  </div>
                    ...
                    {canApprove ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="flex-1 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-900/60 py-2.5 rounded-lg text-xs font-bold transition-colors min-h-[44px] flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50"
                        >
                      ...
                      <div className="p-2.5 bg-slate-50 dark:bg-slate-800/70 border border-slate-200 dark:border-slate-700 rounded-lg text-center text-xs text-slate-500 dark:text-slate-400 font-medium">
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/60 transition-colors">
                        ...
                        <td className="py-3.5 px-4">
                          <span className="font-bold text-orange-800 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/50 px-2 py-0.5 rounded border border-orange-200/60 dark:border-orange-900/60">
                            {item.typeLabel}
                          </span>
                        </td>
                        <td className="py-3.5 px-4">
                        ...
                        <td className="py-3.5 px-4 font-mono font-bold text-slate-900 dark:text-slate-100">
                          {item.amount !== undefined && item.amount > 0 ? (
                            <span className="text-orange-700 dark:text-orange-400">{item.amount} ج.م</span>
                          ) : (
                          ...
                          {canApprove ? (
                            <div className="inline-flex items-center gap-2">
                              ...
                              <button
                                ...
                                onClick={() => handleDecision(item, 'REJECTED')}
                                title="رفض المعاملة"
                                className="px-3 py-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/50 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-900/60 font-bold text-xs transition-colors min-h-[44px] inline-flex items-center gap-1 cursor-pointer disabled:opacity-50"
                              >
                            ...
                            <span className="text-slate-500 dark:text-slate-400 font-mono text-[11px] bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded">
