...
export function CommandPalette() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden z-10 flex flex-col max-h-[80vh] animate-in fade-in-0 zoom-in-95 duration-150">
        {/* Search Input Bar */}
        <div className="flex items-center px-4 py-3.5 border-b border-slate-200 dark:border-slate-800 gap-3">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-hidden"
          />
          {query && (
            <button
              type="button"
          ...
          <kbd className="hidden sm:inline-block bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-[10px] font-mono text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
            ESC للإغلاق
          </kbd>
        </div>

        {/* Results List */}
        <div className="overflow-y-auto p-2 divide-y divide-slate-100 dark:divide-slate-800 flex-1">
          {filteredItems.length === 0 ? (
            <div className="p-8 text-center text-slate-500 dark:text-slate-400 text-xs">
              <Sparkles className="w-6 h-6 text-slate-400 dark:text-slate-500 mx-auto mb-2" />
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl text-right transition-colors cursor-pointer ${
                    isSelected
                      ? 'bg-orange-50 dark:bg-orange-950/40 text-orange-950 dark:text-orange-200'
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {item.badge && (
                          <span className="text-[10px] bg-orange-100 dark:bg-orange-950/60 text-orange-800 dark:text-orange-300 px-1.5 py-0.2 rounded font-bold border border-orange-200/50 dark:border-orange-900/50">
                            {item.badge}
                          </span>
                        )}
                      </div>
                  ...
                  <ArrowRight
                    className={`w-4 h-4 rotate-180 transition-transform ${
                      isSelected ? 'text-orange-600 dark:text-orange-400 -translate-x-1' : 'text-slate-300 dark:text-slate-600'
                    }`}
                  />
                </button>
              );
        ...
        {/* Footer info */}
        <div className="bg-slate-50 dark:bg-slate-950 px-4 py-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-[11px] text-slate-400 dark:text-slate-500">
          <div className="flex items-center gap-2">
            <span>التنقل بالأسهم:</span>
            <kbd className="bg-white dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700 font-mono text-[10px] text-slate-600 dark:text-slate-300">
              ↑
            </kbd>
            <kbd className="bg-white dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700 font-mono text-[10px] text-slate-600 dark:text-slate-300">
              ↓
          ...
          <span className="font-medium text-orange-600 dark:text-orange-400">Al-Saada Smart Command</span>
        </div>
      </div>
    </div>
  );
