...
export const WORKER_SUPERVISOR_COMMANDS: BotCommand[] = [
  { command: 'start', description: '🏠 القائمة الرئيسية وبوابة العامل المشرف' },
  { command: 'cancel', description: '❌ إلغاء المعاملة الحالية والتراجع' },
  { command: 'profile', description: '👤 ملفي الشخصي' },
...
export function getCommandsForRole(role: string, isDualWorker = false): BotCommand[] {
  ...
  switch (role) {
    ...
    case 'GENERAL_ADMIN':
      return GENERAL_ADMIN_COMMANDS;
    case 'FIELD_ADMIN':
      return FIELD_ADMIN_COMMANDS;
    case 'WORKER_SUPERVISOR':
      return WORKER_SUPERVISOR_COMMANDS;
    case 'WORKER':
